...
function App() {
  ...
  const handleModuleQuestions = (e, id) => {
    const newmoduleContent = moduleDetails.map((i) => {
      if (id === i.id) {
        ...
        {courseDetails.map((courseDetail) => (
          <>
            ...
            <IconButton
              ...
              onClick={() => handleRemoveCourseDetailsFields(courseDetail.id)}
            >
              <RemoveIcon />
            </IconButton>
            ...
            {courseDetail.content.map((coursedetaildescription, index) => (
              <>
                <TextareaAutosize
                  style={{
                    width: "85%",
                ...
                <IconButton
                  ...
                  onClick={() =>
                    handleRemoveExtraCourseDescription(courseDetail.id, index)
                  }
                >
                  <RemoveIcon />
                </IconButton>
          ...
          <div key={inputField.id} style={{ marginBottom: "30px" }}>
            {inputField.sections.map((section, index) => (
              <div>
                ...
                <IconButton onClick={() => handleAddFields(inputField.id)}>
                  <AddIcon />
                </IconButton>
              </div>
            ))}
                ...
                {moduleDetail.module_content.map((modcont) => {
                  return (
                    <div>
                      ...
                      />
                      <IconButton
                        disabled={moduleDetail.module_content.length === 1}
                        onClick={() =>
                          handleRemoveSubModule(moduleDetail.id, modcont.id)
                        }
                      >
                        <RemoveIcon />
                      </IconButton>
                      <IconButton
                        onClick={() => handleAddSubModule(moduleDetail.id)}
                      >
                        <AddIcon />
                      </IconButton>
                  ...
                  return (
                    <div>
                      ...
                      <TextareaAutosize
                        ...
                        onChange={(e) =>
                          handleModuleQuestions(e, moduleDetail.id)
                        }
                      />
                      <div>
                        <TextField
